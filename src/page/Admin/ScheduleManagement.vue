    ...
    >
      <div class="text-center">
        ...
        <p class="text-sm text-gray-600">
          Lịch khám ngày <strong>{{ formatDate(selectedSchedule?.date) }}</strong> 
          - <strong>{{ selectedSchedule?.timeTypeText }}</strong> 
          của bác sĩ <strong>{{ selectedSchedule?.doctorName }}</strong> sẽ bị xóa vĩnh viễn. 
...
const handleDelete = async () => {
  try {
    ...
    const response = await scheduleApi.deleteSchedule(selectedSchedule.value.id)
