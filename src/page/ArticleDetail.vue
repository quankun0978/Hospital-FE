      ...
      <template v-else-if="article">
        ...
        <main>
          ...
          <div class="bg-white rounded-lg p-6 mb-6 shadow-sm">
            ...
            <!-- Mô tả ngắn -->
            <div v-if="article.description" class="text-gray-700 text-lg leading-relaxed mb-6 bg-gray-50 p-4 rounded-lg border-l-4 border-primary">
              {{ article.description }}
            </div>
          ...
          <div v-if="relatedArticles.length > 0" class="mt-8">
            <h2 class="text-xl font-bold text-gray-900 mb-4">Bài viết liên quan</h2>
              ...
              >
                <router-link :to="`/articles/${relatedArticle.slug}`">
                  <div class="h-48 bg-slate-200 flex items-center justify-center">
                    <span class="text-gray-500">Hình ảnh</span>
                  </div>
                  <div class="p-4">
                    <h3 class="font-semibold text-gray-900 line-clamp-2 hover:text-primary">
                    ...
                    <div class="flex items-center text-xs text-gray-500 mt-3">
                      <span>{{ relatedArticle.authorName || 'Admin' }}</span>
                      <span class="mx-2">•</span>
...
const fetchRelatedArticles = async () => {
  try {
    ...
    if (response.succeeded && response.data) {
      ...
      relatedArticles.value = response.data.filter(
        item => item.articleId !== article.value?.articleId
      ).slice(0, 3);
    }
  } catch (err) {
